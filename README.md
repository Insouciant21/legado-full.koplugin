...
书籍详情中的 `Change source` 会先列出可搜索的已启用文本书源，再允许修改搜索关键词、
选择候选书籍并确认。换源前会请求新书源目录，以章节标题优先、章节序号兜底迁移阅读进度；
旧书源的章节/封面/生成文件和阅读会话缓存会清理，换源后详情页与目录页会立即使用新书源。
换源页长按带有登录定义的书源可以直接进入该源的登录/Actions，取消搜索或搜索失败会返回
当前换源列表。
